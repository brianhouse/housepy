    ...
    def close(self):
        try:
            try:
                ...
                output = json.dumps(self, indent=4, default=lambda obj: str(obj))               
            else:    
                output = json.dumps(self, indent=4, default=lambda obj: str(obj) if type(obj) != np.ndarray else list(obj))
            self.f.seek(0)
            self.f.write(output)
            self.f.truncate()
            self.f.close()
